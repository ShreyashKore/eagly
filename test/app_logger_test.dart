...
import 'package:logview/features/app_log/app_logger.dart';
...
void main() {
  ...
  test('scoped loggers submit to the shared root store with inherited defaults', () {
    final repositoryLogger = AppLogger(source: 'DeviceRepository');
    final sessionLogger = repositoryLogger.scoped(sessionTag: 'workspace-tab-1');

    sessionLogger.error('Failed to refresh devices', detail: 'adb unavailable');
  ...
  test('entriesWhere and latestEntry can filter to session-scoped errors only', () {
    final root = AppLogger.global;
    final tabLogger = AppLogger(source: 'DeviceSessionService', sessionTag: 'tab-a');
    final otherLogger = AppLogger(source: 'DeviceSessionService', sessionTag: 'tab-b');
    ...
    final tabErrors = root.entriesWhere(sessionTag: 'tab-a', errorsOnly: true);
    expect(tabErrors, hasLength(1));
    expect(tabErrors.single.message, 'Tool error while streaming logs');

    final latestTabError = root.latestEntry(sessionTag: 'tab-a', errorsOnly: true);
